...
public class MusicPlayerTester
{
    public static void main (String[] args)
    {
        ...
        String[] availableFormats = {"Streaming", "DVD", "Cassette", "Vinyl"};
        ...
        while (true)
        {
            ...
            if (physicalVSdigital.equals("physical") || physicalVSdigital.equals("physically"))
            {
                ...
                for (int i = 1; i < availableFormats.length; i++)
                {
                    System.out.println("- " + availableFormats[i]);
                }
            ...
            {
                System.out.println("Available formats:");
                System.out.println("- " + availableFormats[0]);
    ...
    /**
     * Prints a summary of all the music the user selected within a new text file
     * @param usersMusic an ArrayList of MusicPlayer objects representing the user's selected music
     */
    public static void printSummary (ArrayList<MusicPlayer> usersMusic)
    {
        System.out.println("Your Music Summary: ");

        try
        {
            FileWriter write = new FileWriter("music_summary.txt");
            for (MusicPlayer user : usersMusic)
            {
                String summaryLine = user.toString();
                System.out.println(summaryLine);
                write.write(summaryLine + "\n");
            }
            write.close();
            System.out.println("\nSummary saved to 'music_summary.txt'");
        }
        catch (IOException e)
        {
